...
bool RenderProc(LPVOID lpParam)
{
	...
	while (RunLevel)
	{
		...
		// Re-acquire the context as we are in a different thread
		glRC=wglCreateContext(GLDC);
		wglMakeCurrent(GLDC,glRC);

		camera.render();
// 		if (Mouse.Mx != prevMouseY || Mouse.My != prevMouseY)
// 		{
// 			if (Mouse.Mleft)
// 			{
// 				camera.update(Mouse.Mx/800.0, Mouse.My/600.0, 0.f, 0.f, 0.f);
// 				camera.render();
// 			}
// 
// 			prevMouseX = Mouse.Mx;
